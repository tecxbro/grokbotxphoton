...
Reactions resolve to a <TypeTooltip name="Message" type={`interface Message<TPlatform extends string = string, TSender extends User = User, TSpace extends Space = Space> {
content: Content;
...
timestamp: Date;
unsend(): Promise<void>;
}`} /> — keep it as the handle to `unsend()` later. Resolves `undefined` only when the platform does not support reactions.

Use the universal <TypeTooltip name="Emoji" type={`const Emoji: {
readonly love: "❤️";
readonly like: "👍";
readonly dislike: "👎";
readonly laugh: "😂";
...
readonly zombie: "🧟";
readonly zzz: "💤";
}`} /> aliases for iMessage's native tapbacks:
...
Reactions can also be unsent — keep the <TypeTooltip name="Message" type={`interface Message<TPlatform extends string = string, TSender extends User = User, TSpace extends Space = Space> {
content: Content;
direction: "inbound" | "outbound";
edit(newContent: ContentInput): Promise<void>;
readonly id: string;
...
timestamp: Date;
unsend(): Promise<void>;
}`} /> returned by `react()` and pass it to `unsend()`:
